...
from scripts.helpful_script import get_account, get_breed, OPENSEA_URL
...
def set_tokenURI(token_id, nft_contract, tokenURI):
    ...
    tx.wait(1)
    print(
        f"You can see view your NFT at {OPENSEA_URL.format(nft_contract.address, token_id)}"
    )
    print("Please wait up to 20 minutes then refresh")
...
def main():
    ...
    for token_id in range(number_of_collectables):
        ...
        if not advanced_collectable.tokenURI(token_id).startswith("https://"):
            print(f"Settingt tokenURI of {token_id}")
